...
if (isset($_POST['recupereaza-parohie'])) {

  $email = $_POST['email'];
  
  $sql="SELECT id, email FROM parohii WHERE email = '$email'";
  $rezultate = mysqli_query ($conn, $sql);
  ...
  while ($data = mysqli_fetch_assoc($rezultate)){
    ...
    if(mysqli_num_rows($rezultate)==1)
      ...
      } else {echo '<script> location.replace("login/recupereaza-parohie.php?succes=nu"); </script>';}
} 
