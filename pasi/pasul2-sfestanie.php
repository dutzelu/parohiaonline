...
<form method="POST" action="pasul3-sfestanie.php<?php echo '?month=' . $month . '&year=' .  $year . '&pentru=' .  $pentru . '&zi=' . $zi;?> " enctype = "multipart/form-data">
...
foreach ($ore as $key => $ora) {
    ...
    $query = 'SELECT * FROM programari_sfestanie WHERE data_si_ora=? AND parohie_id = ?';
